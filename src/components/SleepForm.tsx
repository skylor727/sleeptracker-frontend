...
export const SleepForm = () => {
  ...
  const redirect = async () => {
    const defaultId = 0;
    await router.push(`/sleeps/${sessionData?.user?.id ?? defaultId}?newSleepAdded=true`);
  };

  const handleSubmit = async (sleepFormData: SleepFormData) => {
    ...
    await sendRequest("POST", "/sleep", sleepFormData);
    void redirect();
  };
          ...
          >
            ...
            <div className="mt-6">
              ...
              <span className="mt-2 block text-sm text-gray-300">
                The calculated time takes into account five complete sleep
                cycles, aiming to optimize your rest and ensure you wake up
